...
import com.jepongdevxyz.vpn.tunnel.DNSTunnelThread;
import com.jepongdevxyz.vpn.tunnel.TunnelManagerThread;
		...
		implements SkStatus.StateListener
{
	...
	public static final String START_SERVICE = "com.jepongdevxyz.vpn:startTunnel";
	...
	public synchronized void startTunnel() {
		...
		try {
			...
			mTunnelThread.start();
				...
				wakeLock.acquire();
wakeLock.release(); 
				SkStatus.logInfo("Wakelock Fast Activo");
	...
	private void showNotification(final String msg, final String channel,
		...
		if (mNotifyBuilder == null) {
			...
			mNotifyBuilder = new Notification.Builder(this)
					.setContentTitle(mConfig.getPrivString("ServerName") +"SocksFast VPN"+mConfig.getPrivString("PayloadName"))
  ...
  if (!mPrefs.network_meter()){
	...
	private void addVpnActionsToNotification(Notification.Builder nbuilder) {
		int flags = Build.VERSION.SDK_INT >= 31 ? PendingIntent.FLAG_IMMUTABLE | PendingIntent.FLAG_CANCEL_CURRENT: PendingIntent.FLAG_CANCEL_CURRENT;
...
connectedVibrate();
	...
	public static PendingIntent getGraphPendingIntent(Context context) {
		...
		int flags = Build.VERSION.SDK_INT >= 31 ? PendingIntent.FLAG_IMMUTABLE | 0: 0;
